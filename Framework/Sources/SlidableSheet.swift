...
class SlidableSheet: NSObject, UIGestureRecognizerDelegate, UIScrollViewDelegate {
    let surfaceView: SlidableSheetSurfaceView
    let backdropView: SlidableSheetBackdropView
    var layoutAdapter: SlidableSheetLayoutAdapter
    var behavior: SlidableSheetBehavior
    ...
    unowned let viewcontroller: SlidableSheetController

    private(set) var state: SlidableSheetPosition = .tip
    ...
    let panGesture: SlidableSheetPanGestureRecognizer
    ...
    init(_ vc: SlidableSheetController, layout: SlidableSheetLayout, behavior: SlidableSheetBehavior) {
        viewcontroller = vc
        surfaceView = vc.view as! SlidableSheetSurfaceView
        backdropView = SlidableSheetBackdropView()
        backdropView.backgroundColor = .black
        backdropView.alpha = 0.0

        self.layoutAdapter = SlidableSheetLayoutAdapter(surfaceView: surfaceView,
                                                        backdropView: backdropView,
                                                        layout: layout)
        self.behavior = behavior

        panGesture = SlidableSheetPanGestureRecognizer()

        if #available(iOS 11.0, *) {
            panGesture.name = "SlidableSheetSurface"
        }
    ...
    func move(to: SlidableSheetPosition, animated: Bool, completion: (() -> Void)? = nil) {
    ...
    private func move(from: SlidableSheetPosition?, to: SlidableSheetPosition?, animated: Bool, completion: (() -> Void)? = nil) {
        if to != .full {
            lockScrollView()
        }
    ...
    private func updateLayout(to target: SlidableSheetPosition?) {
        self.layoutAdapter.activateLayout(of: target)
        self.setBackdropAlpha(of: target)
    }

    private func setBackdropAlpha(of target: SlidableSheetPosition?) {
    ...
    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldRequireFailureOf otherGestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        // Do not begin the pan gesture until any other gestures fail
        // except fo the tracking scrollView's pan gesture.
    ...
    private func shouldScrollViewHandleTouch(_ scrollView: UIScrollView?, point: CGPoint, velocity: CGPoint) -> Bool {
        let grabberBarFrame = CGRect(x: surfaceView.bounds.origin.x,
                                     ...
                                     height: SlidableSheetSurfaceView.topGrabberBarHeight * 2)
    ...
    private func panningChange(with translation: CGPoint) {
        ...
        viewcontroller.delegate?.slidableSheetDidMove(viewcontroller)
    }
    ...
    private func panningEnd(with translation: CGPoint, velocity: CGPoint) {
        ...
        if isRemovalInteractionEnabled, isBottomState {
            ...
            let startRemovalAnimation = {
                ...
                animator.addCompletion({ [weak self] (_) in
                    guard let self = self else { return }
                    self.viewcontroller.delegate?.slidableSheetDidEndRemove(self.viewcontroller)
                })
                animator.startAnimation()
            }
            ...
            if (currentY - posY) != 0 {
                if (safeAreaBottomY - posY) / (currentY - posY) >= 0.5 || velocityVector.dy == vth {
                    viewcontroller.delegate?.slidableSheetDidEndDraggingToRemove(viewcontroller, withVelocity: velocity)
        ...
        viewcontroller.delegate?.slidableSheetDidEndDragging(viewcontroller, withVelocity: velocity, targetPosition: targetPosition)
        viewcontroller.delegate?.slidableSheetWillBeginDecelerating(viewcontroller)
    ...
    private func startInteraction(with translation: CGPoint) {
        ...
        viewcontroller.delegate?.slidableSheetWillBeginDragging(viewcontroller)
    ...
    private func endInteraction(for targetPosition: SlidableSheetPosition) {
    ...
    private func startAnimation(to targetPosition: SlidableSheetPosition, at distance: CGFloat, with velocity: CGPoint) {
    ...
    private func finishAnimation(at targetPosition: SlidableSheetPosition) {
        log.debug("finishAnimation \(targetPosition)")
        self.animator = nil
        self.viewcontroller.delegate?.slidableSheetDidEndDecelerating(self.viewcontroller)
    ...
    private func distance(to targetPosition: SlidableSheetPosition, with translation: CGPoint) -> CGFloat {
        let topY = layoutAdapter.topY
    ...
    private func directionalPosition(with translation: CGPoint) -> SlidableSheetPosition {
        let currentY = getCurrentY(from: initialFrame, with: translation)
    ...
    private func redirectionalPosition(with translation: CGPoint) -> SlidableSheetPosition {
        let currentY = getCurrentY(from: initialFrame, with: translation)
    ...
    private func targetPosition(with translation: CGPoint, velocity: CGPoint) -> (SlidableSheetPosition) {
        let currentY = getCurrentY(from: initialFrame, with: translation)
        let supportedPositions: Set = layoutAdapter.layout.supportedPositions
    ...
    private func targetPosition(from positions: [SlidableSheetPosition], at currentY: CGFloat, velocity: CGPoint) -> SlidableSheetPosition {
...
class SlidableSheetPanGestureRecognizer: UIPanGestureRecognizer {
    override weak var delegate: UIGestureRecognizerDelegate? {
        get {
            return super.delegate
        }
        set {
            guard newValue is SlidableSheet else {
                let exception = NSException(name: .invalidArgumentException,
                                            reason: "SlidableSheetController's built-in pan gesture recognizer must have its controller as its delegate.",
                                            userInfo: nil)
                exception.raise()
                return
            }
